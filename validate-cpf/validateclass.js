...
// Criando Superclass ou classe principal
class Validator {
    constructor(cpf) {
    ...
    // Método da classe Validator, onde é feito as validações principais...
    valida() {
    ...
    // Método da classe Validator, onde não permite que seja inserido nenhuma sequencia numerica, exemplo: (111.111.111-11)
    isSequence() {
        const sequence = this.cpfLimpo[0].repeat(this.cpfLimpo.length);
        return sequence === this.cpfLimpo;
    }

    // Método da classe Validator(), onde é feito o calculo dos digitos verificadores.
    criaChar(cpfParcial) {
        ...
        return char > 9 ? '0' : String(char); // Operação ternária, se o char for maior do que 9 vai retornar 0, caso contrario retorna valor de char.
    }
}
//----------------------------------------------------------------------------------------------------------------------------------------------------
